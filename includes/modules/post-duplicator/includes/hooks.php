...
// Removed plugin_action_links filter as it is not needed for a module
...
function duplicator_submitbox( $post ) {
  ...
  if ( $post_type = get_post_type_object( $post->post_type ) ) {
    ...
    <div class="misc-pub-section misc-pub-duplicator" id="duplicator">
      <a class="m4c-duplicate-post button button-small" href="#" data-postid="<?php echo esc_attr( $post->ID ); ?>" data-posttype="<?php echo esc_attr( $post->post_type ); ?>"><?php printf( esc_html__( 'Duplicate %s', 'wp-genius' ), $post_type->labels->singular_name ); ?></a><span class="spinner" style="float:none;margin-top:2px;margin-left:4px;"></span>
    </div>
    <?php
  }
}
