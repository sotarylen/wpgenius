...
/**
 * Auto Upload Images Module - Image Uploader Class
 * @author Ali Irani <ali@irani.im>
 * @modified for W2P Module System
 */
class AutoUploadImages_ImageUploader
{
    public $post;
    public $url;
    public $alt;
    ...
    {
        $url = $url ?: AutoUploadImagesModuleModule::getOption('base_url');
    ...
    public function validate()
    {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Auto Upload Images: Validating image URL ' . $this->url);
            error_log('Auto Upload Images: Image host ' . $url . ', site host ' . $site_url);
        }

        if ($url === $site_url || !$url) {
            // 添加调试信息
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Auto Upload Images: Image is local or host is empty');
            }
            return false;
        }

        if ($urls = AutoUploadImagesModuleModule::getOption('exclude_urls')) {
            ...
            foreach ($exclude_urls as $exclude_url) {
                if ($url === self::getHostUrl(trim($exclude_url))) {
                    // 添加调试信息
                    if (defined('WP_DEBUG') && WP_DEBUG) {
                        error_log('Auto Upload Images: Image host ' . $url . ' is in exclude list');
                    }
                    return false;
                }
            }
        ...
        // 添加调试信息
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Auto Upload Images: Image validation passed');
        }
        return true;
    }
    ...
    {
        $filename = trim($this->resolvePattern(AutoUploadImagesModuleModule::getOption('image_name', '%filename%')));
    ...
    {
        return $this->resolvePattern(AutoUploadImagesModuleModule::getOption('alt_name'));
    }
    ...
    public function save()
    {
        if (!$this->validate()) {
            // 添加调试信息
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Auto Upload Images: Image validation failed for ' . $this->url);
            }
            return null;
        }
        ...
        // 添加调试信息
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Auto Upload Images: Attempting to download image ' . $this->url);
        }
        ...
        if (is_wp_error($image)) {
            // 添加调试信息
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Auto Upload Images: Download failed for ' . $this->url . ': ' . $image->get_error_message());
            }
            return null;
        }
        ...
        // 添加调试信息
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Auto Upload Images: Successfully downloaded image ' . $this->url . ' to ' . $image['url']);
        }
    ...
    public function downloadImage($url)
    {
        ...
        $args = [
            ...
            'timeout' => 30,
            'sslverify' => false, // often necessary for some misconfigured external sites
        ];
        ...
        $attach_id = $this->attachImage($image);
        ...
        if ($this->isNeedToResize() && ($resized = $this->resizeImage($image))) {
            ...
            // Re-attach or update metadata if needed? 
            // Usually resizeImage just creates files, attachImage handles metadata. 
            // If we replace the main file, we might need to update attachment.
            // But here let's stick to base logic, just ensuring valid ID return.
            $this->attachImage($image); // Re-run to update metadata with new size if replaced? 
                                      // Actually existing code called it twice too.
        }
    ...
    public function attachImage($image)
    {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            // error_log('AUI Attach Debug: Attachment ID: ' . $attach_id . ', Post ID: ' . $this->post['ID']);
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            // error_log('AUI Attach Debug: Metadata updated. Result: ' . ($attach_data ? 'Success' : 'Failed'));
        }
    ...
    {
        $width = AutoUploadImagesModuleModule::getOption('max_width');
        $height = AutoUploadImagesModuleModule::getOption('max_height');
    ...
    {
        return AutoUploadImagesModuleModule::getOption('max_width') || AutoUploadImagesModuleModule::getOption('max_height');
    }
