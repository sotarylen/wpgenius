...
final class W2P_Image_Watermark {
    ...
    public function __construct() {
        ...
        $this->options['watermark_image'] = array_merge($this->defaults['options']['watermark_image'], isset($options['watermark_image']) ? $options['watermark_image'] : []);
        $this->options['image_protection'] = array_merge($this->defaults['options']['image_protection'], isset($options['image_protection']) ? $options['image_protection'] : []);
        $this->options['backup'] = array_merge($this->defaults['options']['backup'], isset($options['backup']) ? $options['backup'] : []);

        include_once(W2P_IMAGE_WATERMARK_PATH . 'includes/class-update.php');
        ...
        // filters
        add_filter('plugin_action_links_' . W2P_IMAGE_WATERMARK_BASENAME, [$this, 'plugin_settings_link']);
    ...
    private function define_constants() {
        // Constants are defined in the module class
    }
    ...
    public function activate_watermark() {
        // add default options
        add_option('w2p_image_watermark_options', $this->defaults['options'], null, false);
        add_option('w2p_image_watermark_version', $this->defaults['version'], null, false);
    }
    ...
    public function deactivate_watermark() {
        ...
        if ($this->options['watermark_image']['deactivation_delete'])
            delete_option('w2p_image_watermark_options');
    }
    ...
    public function update_plugin() {
        ...
        $db_version = get_option('w2p_image_watermark_version');
        ...
        if ($db_version != false) {
            if (version_compare($db_version, '1.5.0', '<')) {
                ...
                add_option('w2p_image_watermark_options', $options, null, false);
                add_option('w2p_image_watermark_version', $this->defaults['version'], null, false);
            }
        }
    }
    ...
    public function admin_enqueue_scripts($page) {
        ...
        wp_register_style('w2p-watermark-style', plugin_dir_url(WP_GENIUS_FILE) . 'assets/css/modules/core.css', [], $this->defaults['version']);
        ...
        if ($page === 'settings_page_watermark-options') {
            ...
            wp_add_inline_script('w2p-image-watermark', 'var iwArgsUpload = ' . wp_json_encode($script_data) . ";\n", 'before');
            ...
            wp_add_inline_script('w2p-image-watermark', 'var iwArgsSettings = ' . wp_json_encode($script_data) . ";\n", 'before');
        ...
        if ($pagenow === 'upload.php') {
            if ($this->options['watermark_image']['manual_watermarking'] == 1 && current_user_can('upload_files')) {
                ...
                wp_add_inline_script('w2p-image-watermark', 'var iwArgsMedia = ' . wp_json_encode($script_data) . ";\n", 'before');
            }
        ...
        if ($this->options['watermark_image']['manual_watermarking'] == 1) {
            ...
            wp_add_inline_script('w2p-image-watermark', 'var iwArgsImageActions = ' . wp_json_encode($script_data) . ";\n", 'before');
        }
    }
    ...
    public function wp_enqueue_scripts() {
        ...
        if (apply_filters('iw_block_right_click', (bool) $right_click) === true) {
            ...
            wp_add_inline_script('w2p-image-watermark', 'var iwArgsNoRightClick = ' . wp_json_encode($script_data) . ";\n", 'before');
        }
    }
